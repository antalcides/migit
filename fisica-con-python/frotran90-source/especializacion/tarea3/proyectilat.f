...
c     **Declaraci¢n de variables**
      ...
      real v0,vx,vy,v,v1,dt,g,teta,tet,cd,m,y0,e
      ...
      !m es la masa,y0 es la altura del nivel del mar
...
c     **M‚todo de Euler para las ecuaciones**
      ...
      do while (i.LT.imax)
        ...
        y(i)=y(i-1)+(vy*dt)
        e=exp(-y(i)/y0)
        vy=vy-(g*dt)-(e*cd*v*vy*dt)/m

        vyj(i)=vy
        vx=vx-(e*cd*v*vy*dt)/m
        t(i)=i*dt
        v1=(vx**2)+(vy**2)
        v=sqrt(v1)
        if(y(i).GT.ymax)then
...
c     **abrir los archivos y escribir los datos**
      open(20,file='posicionat.dat',status='unknown')
